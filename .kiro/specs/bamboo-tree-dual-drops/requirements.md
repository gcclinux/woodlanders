...
This feature implements a dual-item drop system for BambooTree destruction. When a BambooTree is destroyed, it will drop two distinct items (BambooStack and BambooSapling) at the tree's position, positioned a few pixels apart. This extends the existing single-item drop pattern used by AppleTree and BananaTree to support multiple item drops from a single tree.
...
- **BambooSapling**: An item entity representing a young bamboo plant, extracted from the sprite sheet at coordinates (192, 128) with 64x64 dimensions
...
**User Story:** As a player, I want BambooTree to drop two items when destroyed, so that I can collect both BambooStack and BambooSapling resources

#### Acceptance Criteria

1. WHEN a BambooTree health reaches zero, THE Game System SHALL spawn one BambooStack item at the tree's base position
2. WHEN a BambooTree health reaches zero, THE Game System SHALL spawn one BambooSapling item at the tree's base position offset by 8 pixels horizontally from the BambooStack
...
2. THE MyGdxGame Class SHALL render BambooSapling items at 32x32 pixels on screen
3. THE BambooStack Class SHALL extract its texture from sprite sheet coordinates (128, 128) with source dimensions 64x64
4. THE BambooSapling Class SHALL extract its texture from sprite sheet coordinates (192, 128) with source dimensions 64x64
5. THE MyGdxGame Class SHALL maintain separate collections for BambooStack and BambooSapling items to enable independent rendering and collision detection
...
2. WHEN the player's collision box overlaps with a BambooSapling item, THE Player Class SHALL remove the BambooSapling from the game world
