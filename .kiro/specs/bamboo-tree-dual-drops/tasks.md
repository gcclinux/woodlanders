...
- [x] 1. Add bamboo item collections to MyGdxGame
  ...
  - Add `Map<String, BambooSapling> bambooSaplings` field declaration after bambooStacks
...
- [x] 2. Implement bamboo item rendering in MyGdxGame
  ...
  - Create `drawBambooSaplings()` method that renders BambooSapling items at 32x32 pixels with viewport culling
...
- [x] 3. Wire bamboo item collections to Player class
  - Add `bambooStacks` and `bambooSaplings` field declarations in Player class (around line 53)
  - Create `setBambooStacks()` and `setBambooSaplings()` setter methods in Player class (around line 96)
...
- [x] 4. Implement dual-item drop logic for BambooTree destruction
  ...
  - Create BambooSapling at position offset by 8 pixels horizontally using key `targetKey + "-babybamboo"`
...
- [x] 6. Implement BambooSapling pickup detection and handling
  - Create `checkBambooSaplingPickups()` method in Player class that iterates bamboo saplings and checks collision
  - Create `pickupBambooSapling()` method that handles single-player pickup (dispose and remove) and multiplayer pickup (send message)
  - Add call to `checkBambooSaplingPickups()` in Player's `update()` method after `checkBambooStackPickups()`
