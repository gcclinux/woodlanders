...
public class FenceBuildingManager {
    ...
    public void update(float deltaTime) {
        ...
        // (Player class handles B key when already in building mode for fence
        // navigation)
    ...
    private void processBuildingInput() {
        ...
        // Constrain fence placement to be within reasonable distance from camera
        // (viewport area)
        ...
        if (leftClicked && !disableDirectMouseInput) {
            System.out.println(
                    "[FenceBuildingManager] Left click detected at grid (" + gridPos.x + ", " + gridPos.y + ")");
        ...
        if (rightClicked) {
            System.out.println(
                    "[FenceBuildingManager] Right click detected at grid (" + gridPos.x + ", " + gridPos.y + ")");
            // Debug: Check what fence pieces exist
            FencePiece existingPiece = structureManager.getFencePiece(gridPos);
            System.out.println("DEBUG: Fence piece at (" + gridPos.x + ", " + gridPos.y + "): "
                    + (existingPiece != null ? existingPiece.getType() : "null"));
    ...
    public boolean placeFenceSegment(int gridX, int gridY) {
        ...
        FencePlacementValidator.ValidationResult result = validator.validatePlacement(gridPos, selectedMaterialType,
                playerId);
        ...
        try {
            System.out.println("[FenceBuildingManager] Attempting to place fence at grid (" + gridPos.x + ", "
                    + gridPos.y + ") with material " + selectedMaterialType + " for owner " + playerId);
            ...
            if (placedPiece != null) {
                System.out.println("[FenceBuildingManager] Fence piece created: " + placedPiece.getType()
                        + " at world (" + placedPiece.getX() + ", " + placedPiece.getY() + ") with ID " + fenceId);

                // Send fence place message to server in multiplayer mode
                wagemaker.uk.network.GameClient gameClient = player.getGameClient();
                System.out.println(
                        "DEBUG: Sending fence place message - gameClient=" + (gameClient != null ? "exists" : "null") +
    ...
    private void displayPlacementSuccess(FencePiece placedPiece, Point gridPos) {
        ...
        // In a full implementation, this would play placement sounds and show visual
        // feedback
    }
    ...
    public boolean removeFenceSegment(int gridX, int gridY) {
        ...
        FencePlacementValidator.ValidationResult result = validator.validateRemoval(gridPos, playerId);
        ...
        // Store material type for return (in a full implementation, this would be
        // stored per piece)
        ...
        try {
            ...
            if (removedPiece != null) {
                ...
                wagemaker.uk.network.GameClient gameClient = player.getGameClient();
                System.out.println(
                        "DEBUG: Sending fence remove message - gameClient=" + (gameClient != null ? "exists" : "null") +
                                ", connected=" + (gameClient != null ? gameClient.isConnected() : "N/A"));
                if (gameClient != null && gameClient.isConnected()) {
                ...
                // Update adjacent piece connections (handled by
                // structureManager.removeFencePiece)
                // The structure manager automatically updates connections when pieces are
                // removed
    ...
    private void displayRemovalSuccess(FencePiece removedPiece, Point gridPos, FenceMaterialType materialReturned) {
        ...
        // In a full implementation, this would play removal sounds and show visual
        // feedback
    }
     ...
     * Finds the closest fence piece, identifies all connected pieces, and removes
     * them.
    ...
    public void clearNearestEnclosure() {
        ...
        if (closestPos != null) {
            ...
            if (closestPiece == null || !playerId.equals(closestPiece.getOwnerId())) {
                System.out.println(
                        "[FenceBuildingManager] Cannot clear enclosure - closest fence piece is not owned by current player");
                return;
            }
            ...
            if (!connectedPoints.isEmpty()) {
                ...
                if (ownedPieces.isEmpty()) {
                    System.out.println(
                            "[FenceBuildingManager] Cannot clear enclosure - no pieces owned by current player");
                    return;
                }

                System.out.println("[FenceBuildingManager] Clearing " + ownedPieces.size() + " owned pieces out of "
                        + connectedPoints.size() + " total pieces nearest to " + closestPos);
                    ...
                    if (removedPiece != null) {
                        ...
                        if (gameClient != null && gameClient.isConnected()) {
                            ...
                            if (fenceId == null) {
                                ...
                                System.out
                                        .println("WARNING: Removed fence piece had no ID, using fallback: " + fenceId);
                            }

                            wagemaker.uk.network.FenceRemoveMessage fenceMsg = new wagemaker.uk.network.FenceRemoveMessage(
                                    playerId, fenceId, pos.x, pos.y, selectedMaterialType, playerId);
                            gameClient.sendMessage(fenceMsg);
                            System.out.println("DEBUG: Sent FenceRemoveMessage for enclosure clear - fence ID: "
                                    + fenceId + " at (" + pos.x + ", " + pos.y + ")");
                        }
                ...
                if (materialProvider != null && removedCount > 0) {
                    ...
                    System.out.println(
                            "Returned " + removedCount + " " + selectedMaterialType.getDisplayName() + " materials");
                }
    ...
    public static class EnclosureRequirement {
        ...
        public String toString() {
            return isValid() ? "Pieces needed: " + piecesNeeded : "Error: " + errorMessage;
        }
    }
}