...
/**
 * Core biome management system that determines which biome applies at any world
 * coordinate.
 * This class handles biome zone configuration, distance calculations, texture
 * caching,
 ...
 * 
 * Requirements: 1.2 (distance calculation), 1.3 (seamless rendering), 1.5 (mode
 * consistency),
 * 2.1 (performance), 2.2 (consistent depth), 4.1 (coordinate-based), 4.2
 * (deterministic)
 */
public class BiomeManager {
     ...
     * Call initialize() after construction to set up biome zones and generate
     * textures.
     */
    public BiomeManager() {
    ...
    public void initialize() {
        ...
        if (com.badlogic.gdx.Gdx.graphics != null && isRenderThread) {
            try {
                ...
                System.err
                        .println("BiomeManager: Texture generation failed, entering headless mode: " + e.getMessage());
                headlessMode = true;
            }
        } else {
     ...
     * 
     *         Requirements: 1.3 (seamless rendering), 2.1 (performance), 4.1
     *         (coordinate-based)
     */
    public Texture getTextureForPosition(float worldX, float worldY) {
        ...
        if (headlessMode) {
            throw new IllegalStateException(
                    "Cannot get textures in headless mode (unit tests). Use getBiomeAtPosition() instead.");
        }
     ...
     * 
     *         Requirements: 1.1 (water only in sand), 1.2 (biome type
     *         exhaustiveness), 1.3 (buffer distance), 4.1 (coordinate-based), 4.2
     *         (deterministic)
     */
     ...
     * 
     *         Requirements: 2.1 (50% grass, 50% sand base distribution), 2.3
    ...
    public BiomeType getBaseBiomeAtPosition(float worldX, float worldY) {
        ...
        return normalizedNoise > BiomeConfig.SAND_BASE_THRESHOLD ? BiomeType.SAND : BiomeType.GRASS;
    }
    ...
    private boolean isInSandPatch(float worldX, float worldY) {
        ...
        // Add periodic variation based on distance to create "rings" of varying sand
        // density
     ...
     * 
     *         Requirements: 1.4 (biome distribution), 1.5 (contiguous lake
     *         regions), 4.1 (coordinate-based)
     */
    @SuppressWarnings("unused")
    private boolean isInWaterPatch(float worldX, float worldY) {
        ...
        // Threshold for water - adjusted to maintain ~15% coverage with new noise
        // scales
        return waterProbability > BiomeConfig.WATER_NOISE_THRESHOLD;
    }

    /**
     * Calculates the Euclidean distance from the spawn point (0,0) to a given
     * position.
     * 
     ...
     * Uses multiple octaves of simplex-like noise to create organic, wavy
     * boundaries
     ...
     * @param x The x-coordinate in world space
     * @param y The y-coordinate in world space
     * @return A noise offset value to add to the distance calculation (typically
     *         -1500 to +1500)
     * 
     ...
     * 
     * Requirements: 2.1 (deterministic base biome calculation), 1.1 (deterministic
     * water placement)
     */
    ...
    /**
     * Generates noise for base biome calculation using the base biome Random
     * instance.
     ...
     * @return A noise value between -1.0 and 1.0
     * 
     *         Requirements: 2.1 (deterministic base biome calculation), 2.3
     *         (noise-based distribution)
     */
    ...
    /**
     * Generates noise for water-in-sand calculation using the water-in-sand Random
     * instance.
     * This creates deterministic noise patterns for water placement within sand
     * areas.
     * 
     * @param x The scaled x-coordinate for noise sampling
     * @param y The scaled y-coordinate for noise sampling
     * @return A noise value between -1.0 and 1.0
     * 
     *         Requirements: 1.1 (deterministic water placement), 1.4 (water
     *         coverage in sand)
     */
     ...
     * 
     *         Requirements: 1.1 (water only in sand), 1.3 (buffer distance), 1.4
     *         (40% coverage)
     */
     ...
     * 
     *         Requirements: 1.3 (water maintains buffer distance from grass), 3.1
    ...
    private boolean isValidBeachBufferStandard(float worldX, float worldY) {
        ...
        // Performance optimization: Check cardinal directions first (most likely to
        // find grass)
        ...
        for (int i = 0; i < innerSamplePoints; i++) {
            ...
            // Early exit: If any point in the inner buffer zone is grass, reject
            // immediately
            if (getBaseBiomeAtPosition(checkX, checkY) == BiomeType.GRASS) {
                return false;
            }
     ...
     * Used for debugging and validation purposes to understand buffer zone
     * violations.
     * This method performs a comprehensive search in a wider area around the point.
     * 
     * @param worldX The x-coordinate in world space
     * @param worldY The y-coordinate in world space
     * @return The minimum distance to grass in pixels, or Float.MAX_VALUE if no
     *         grass found
     * 
        ...
        for (float dx = -searchRadius; dx <= searchRadius; dx += sampleResolution) {
            for (float dy = -searchRadius; dy <= searchRadius; dy += sampleResolution) {
                ...
                if (getBaseBiomeAtPosition(checkX, checkY) == BiomeType.GRASS) {
                    ...
                    // Early exit optimization - if we found grass very close, no need to search
                    // further
     ...
     * This method is used to validate that the current thresholds produce the
     * desired distribution.
     * 
     * @param sampleSize  The number of random coordinates to sample
     * @param maxDistance The maximum distance from spawn to sample (0 for
     *                    unlimited)
     ...
     * This helps tune the WATER_IN_SAND_THRESHOLD to achieve the target 40%
     * coverage.
     * 
     ...
     * @return A map containing performance metrics in nanoseconds and operations
     *         per second
     */
